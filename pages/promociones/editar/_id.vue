      ...
      <v-stepper v-model="e1">
        ...
        <v-stepper-items>
          <v-stepper-content step="1">
            ...
            <v-btn color="error" class="mr-4" @click="eliminar">
              Eliminar Promoción</v-btn
            >
            <v-btn color="success" @click="validar1">Siguiente</v-btn>
          </v-stepper-content>
...
export default {
  ...
  async mounted() {
    ...
    this.promo = prom.data.data.filter(
      (i) => i.id_promo === Number(this.$route.params.id) && i.estado === 0
    )

    this.nombre = this.promo[0].nombrePromo
    this.descripcion = this.promo[0].descripcionPromo
    this.valor = this.promo[0].valorDescuento
    this.tipoPromo = this.promo[0].idTipoPromo
    this.condiciones = this.promo[0].condiciones

    const fecha1 = this.promo[0].fechaInicio.split(' ')
    this.fromDateValI = fecha1[0]
    this.fromTimeValI = fecha1[1]

    const fecha2 = this.promo[0].fechaFin.split(' ')
    this.fromDateValF = fecha2[0]
    this.fromTimeValF = fecha2[1]
    this.url = this.promo[0].urlBannerImagen
    this.auxurl = this.promo[0].urlBannerImagen

    const propro = await axios.get(
      env.endpoint + '/promoProducto.php?id=' + this.loggedInUser.idComercio
    )

    const auxp = propro.data.data.filter(
      (e) => e.idPromocion === Number(this.$route.params.id) && e.estado === 0
    )

    this.productoPromo = []
    this.auxeli = []

    auxp.forEach((element) => {
      this.productoPromo.push(element.idProducto)
      this.auxeli.push(element.idProducto)
    })
  },
  computed: {
  ...
  methods: {
    ...
    async registrar() {
      ...
      const pr = await axios.put(env.endpoint + '/promocionComercio.php', json)
      ...
      if (pr.data.code === 200) {
        this.productoPromo.forEach(async (element) => {
          if (this.auxeli.includes(element) === false) {
            await axios.post(env.endpoint + '/promoProducto.php', {
              idProducto: element,
              idPromocion: Number(this.$route.params.id),
            })
          }
        })

        this.auxeli.forEach(async (element) => {
          if (this.productoPromo.includes(element) === false) {
            await axios.patch(env.endpoint + '/promoProducto.php', {
              id: element,
              campo: 'estado',
              dato: 1,
            })
    ...
    validar1() {
      ...
      if (this.valid) {
        ...
        this.error = true
        this.error_msg = 'Llene correctamente los datos'
      }
    },
    ...
    async eliminar() {
      ...
      await axios.patch(env.endpoint + '/promocionComercio.php', {
      ...
      this.error = true
      this.error_msg = 'Se eliminó correctamente la promoción'
