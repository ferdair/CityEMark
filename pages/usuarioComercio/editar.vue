    ...
    <div>
      <v-container>
        <v-card class="elevation-12">
          ...
          <v-divider></v-divider>
          <v-card-actions>
            <v-btn to="/usuarioComercio" nuxt color="success"
              >Registrarse</v-btn
            >
...
export default {
  ...
  async created() {
    ...
    this.provincias = data.provincia.filter((m) => m.nombreProv !== 'No State')
  ...
  methods: {
    ...
    async changeCiudad() {
      ...
      this.ciudades = data.ciudad.filter((m) => m.provincia === this.provincia)
    },
    changeCiudad2() {
      alert(this.ciudad)
    },
